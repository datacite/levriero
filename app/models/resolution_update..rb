class ResolutionUpdate < Base
  LICENSE = "https://creativecommons.org/publicdomain/zero/1.0/"
  LOGGER = Logger.new(STDOUT)


  def self.import(_options={})
    resolution_update = ResolutionUpdate.new
    resolution_update.queue_jobs 
  end
  ...
  def process_data options={} 
    ...
    messages.each do |message|
      ...
      ResolutionUpdateParseJob.perform_later(report_id, options)
  ...
  def get_query_url _options={}
    queue_url = sqs.get_queue_url(queue_name: "#{Rails.env}_resolution" ).queue_url
  ...
  def queue_jobs(options={})
    ...
    if total < 1
      text = "No works found in the resolution Reports Queue."
    end
    ...
    text = "#{queued} reports queued out of #{total} for resolution Reports Queue"
  ...
  def self.format_event type, data, options={}
    ...
    fail "Access token missing." if ENV['DATACITE_RESOLUTION_SOURCE_TOKEN'].blank?
    ...
    { "message-action" => "create",
      ...
      "source-id" => "datacite-resolution",
      "source-token" =>ENV['DATACITE_RESOLUTION_SOURCE_TOKEN'],

...
class ZbmathArticle
  ...
  def self.import_by_day(options = {})
    ...
    (from_date..until_date).each do |d|
      ZbmathArticleImportByDayJob.perform_later(from_date: d.strftime("%Y-%m-%dT00:00:00Z"),
  ...
  def self.import(options = {})
    ...
    if options[:filename]
      ...
      Rails.logger.info "Getting ZBMath Article record identifiers updated from #{from_date} until #{until_date} and writing to #{filename}."
    ...
    begin
      ...
      client.list_identifiers(metadata_prefix: "datacite_articles", from: options[:from],
        ...
        if options[:filename]
          ...
          Rails.logger.info "Queueing ZBMath Article record #{record.identifier} for import."
          ZbmathArticleImportJob.perform_later(record.identifier)
        end
        count += 1
      end
    rescue OAI::NoMatchException
      Rails.logger.warn "No ZBMath Article records updated between #{options[:from]} and #{options[:until]}."
      return nil
    rescue OAI::Exception => e
      if e.message.include?("Gateway Timeout")
        Rails.logger.warn "Got Gateway Timeout error from OAI server for ZBMath Article records updated between #{options[:from]} and #{options[:until]}."
  ...
  def get_zbmath_record(identifier)
    ...
    begin
      ...
      Rails.logger.warn "ZBMath Article records #{identifier} not found in the OAI server."
      nil
    rescue OAI::Exception => e
      if e.message.include?("Gateway Timeout")
        Rails.logger.warn "Got Gateway Timeout error from OAI server for ZBMath Article record #{identifier}."
        raise RuntimeError("Gateway Timeout")
      else
        raise e
      end
  ...
  def self.process_zbmath_record(identifier)
    ...
    if record.nil?
      Rails.logger.warn "No valid record returned from OAI server for ZBMath Article record #{identifier}."
      return nil
    end
    ...
    begin
      ...
      Rails.logger.error "Error processing ZBMath Article record #{identifier}: #{e.message}"
      raise
    end
    ...
    Array.wrap(items).each do |item|
      if item["subj_id"].present? && item["obj_id"].present? && item["relation_type_id"].present?
        data = {
          "data" => {
            "id" => item["id"],
            "type" => "events",
        ...
        Rails.logger.warn("ZBMath Article record #{identifier} generated an invalid event: #{item}")
      end
    end
    items.length
  end

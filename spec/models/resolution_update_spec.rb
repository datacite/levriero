...
# describe ResolutionUpdate, type: :model, vcr: true do
...
#         response = ResolutionUpdate.get_data(message,options)
...
#     context "when the resolution event was NOT found" do
#       it "should return errors" do
#         body = File.read(fixture_path + 'resolution_update_nil.json')
#         result = OpenStruct.new(body:  JSON.parse(body) )
#         expect(ResolutionUpdate.parse_data(result)).to be_a(Array)
#         expect(ResolutionUpdate.parse_data(result)).to eq([{"status"=>"404", "title"=>"The resource you are looking for doesn't exist."}])
#       end
#     end


#     context "when the resolution report was NOT found" do
#       it "should return errors" do
#         body = File.read(fixture_path + 'resolution_update_nil.json')
#         result = OpenStruct.new(body:  JSON.parse(body) )
#         expect(ResolutionUpdate.parse_data(result)).to be_a(Array)
...
#         body = File.read(fixture_path + 'resolution_update_1.json')
#         result = OpenStruct.new(body: JSON.parse(body), url:"https://api.test.datacite.org/reports/5cac6ca0-9391-4e1d-95cf-ba2f475cbfad"  )
#         response = ResolutionUpdate.parse_data(result)
...
#       body = File.read(fixture_path + 'resolution_events.json')
#       result = JSON.parse(body).first.to_json
#       options = { }
#       # expect(ResolutionUpdate.push_item(result, options)).to eq(true)
#     end
#   end

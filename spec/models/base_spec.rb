...
describe Base, type: :model, vcr: true do
  ...
  context "get_datacite_metadata" do
    it "fetch metadata scholarly-article" do
      ...
      expect(response["author"].first).to eq("@type"=>"Person", "familyName"=>"Fenner", "givenName"=>"Martin", "name"=>"Fenner, Martin")
    ...
    it "fetch metadata with funding information" do
      ...
      expect(response["author"].first).to eq("@id"=>"https://orcid.org/0000-0002-7352-517X", "@type"=>"Person", "familyName"=>"Hallett", "givenName"=>"Richard", "name"=>"Hallett, Richard")
      expect(response["publisher"]).to eq("@type"=>"Organization", "name"=>"DataCite")
